# Final Hardware Smoke Test Checklist — All 20 Apps

Docs only. This is the final counterpart to
`docs/PHASE2F_HARDWARE_SMOKE_TEST_CHECKLIST.md` (which remains
unmodified and still valid on its own for the 19 apps it covers) — read
`docs/PHASE2A_HARDWARE_SMOKE_TEST_PLAN.md` first for scope, safety
rules, and global fail conditions before running any step below; that
plan is batch-agnostic and applies unchanged to this checklist. **No
hardware testing has been performed as part of writing this
checklist.**
...
**Final hardware gate cross-reference**: `tools/final_hardware_gate.ps1
-Mode HardwareAssisted` automates the non-GUI preconditions
(branch/commit check, artifact hash verification, device detection,
qFlipper detection, excluded-asset absence check, `barcode_gen`
source-fix preservation check, `fcc_id_lookup` LICENSE preservation
check, FCC database accidental-presence check, and the flash-
confirmation gate) and lists every one of the 20 apps below as
`REQUIRES_HUMAN_OBSERVATION`, pointing back to this checklist — it does
not and cannot perform any of the steps in this document itself. See
`docs/FINAL_HARDWARE_ASSISTED_VALIDATION.md` for what that gate does
and does not validate.

No unsafe hardware/radio/security tests (Sub-GHz/RF, NFC, RFID/LFRFID,
iButton, BadUSB/HID injection, BLE, GPIO, IR, or any cloning/brute-force/
jamming/bypass/credential/unauthorized-access behavior) are included
anywhere in this checklist — none of the 20 apps in this final baseline
requires them, and this project's own safety-exclusion policy forbids
adding them here regardless.

Menu path prefix for all 20 apps: **Main Menu → Apps → \<category\>**
(category noted per app below), then the app's own name.

Sections 1–19 (`network_subnet`, `programmer_calc`, `vin_decoder`,
`flipper95`, `chess`, `flipfetch`, `quadratic_solver`, `sudoku`,
`sd_info`, `docviewlite`, `resistors`, `crypto_dictionary`, `2048`,
`image_viewer`, `boilerplate`/`fap_boilerplate`,
`minesweeper`/`minesweeper_redux`, `qrcode`, `hex_viewer`,
`barcode_gen`/`barcode_app`) are identical in content to
`docs/PHASE2F_HARDWARE_SMOKE_TEST_CHECKLIST.md` (which is itself
identical for sections 1-16 to `docs/PHASE2E_HARDWARE_SMOKE_TEST_CHECKLIST.md`)
— reproduced here so this document is a complete, standalone checklist
for the full final 20-app baseline, not a fragment. Section 20 is new,
for `fcc_id_lookup`, the app added via its own dedicated one-app import
and baseline finalization phases.
...
## 14. `image_viewer` (Image Viewer)

- **Category / menu path**: Apps → Tools → "Image Viewer"
- **Launch path**: From the Tools submenu, select "Image Viewer" and press
  OK.
- **Expected screen on launch**: A file-browser dialog (the standard
  Flipper file-browser widget) prompting the user to select an image
  file — **no bundled example image is offered or referenced**, since
  `example_images/` was deliberately excluded at import (see
  `docs/PHASE2E_THIRD_PARTY_NOTICES.md`).
- **Minimum navigation test**: Navigate the file browser to any `.bm`
  image file present on the SD card (supplied by the tester, not bundled
  with this app), and confirm selecting it opens the image viewer.
- **One normal input test**: With an image open, use whatever
  pan/zoom/next-image controls the app exposes (per its own on-screen
  hints) and confirm the display responds correctly.
- **One invalid/edge input test**: Attempt to open a non-image file (e.g.
  a `.txt` file) via the same file browser, or a corrupted/malformed
  `.bm` file if one is available, and confirm the app shows a clear
  error/refusal rather than crashing or freezing.
- **Exit behavior**: Press Back from the image view to return to the file
  browser, and again to return to the Tools menu; confirm both happen
  cleanly.
- **`example_images/` on-device absence check (specific to this app —
  required, not optional)**: Using the device's own file browser (or a
  card reader), confirm **no `cat.bm`, `dolphin.bm`, or `spongebob.bm`
  file exists anywhere on the SD card** as a result of this app's own
  installation — these files were excluded at import time specifically
  because `spongebob.bm` was confirmed to depict a recognizable
  trademarked/copyrighted cartoon character with no attribution or
  redistribution-rights evidence. This step confirms the exclusion holds
  on the real, flashed device, not just in the repository source.
- **SD-card inspection (specific to this app)**: After opening and
  viewing one or more images, confirm **no new file and no modification**
  appears anywhere on the SD card — source confirms this app opens
  whatever file the user selects via `FSAM_READ` only, with no
  `storage_file_write` call anywhere in its source
  (`docs/PHASE2E_1_SOURCE_LICENSE_VERIFICATION.md`,
  `docs/PHASE2E_2_GO_NO_GO.md`). This step exists to confirm that holds on
  real hardware, not just in source.
- **Expected app-private storage path**: **None — confirmed read-only.**
  This app reads a user-selected file only; it is not expected to write
  any persistent file, app-private or otherwise. Any write of any kind is
  a fail condition, not merely an "unexpected path" one.
- **Pass criteria**: App appears in Tools menu; launches; file browser
  opens with no bundled example image offered; a supplied `.bm` image can
  be selected and viewed; pan/zoom/next-image controls (if present) work;
  a non-image or corrupted file is handled without crash/freeze; Back
  returns cleanly through both screens; **no `cat.bm`/`dolphin.bm`/
  `spongebob.bm` exists anywhere on the SD card**; **no file on the SD
  card is created or modified by using this app**.
- **Fail criteria**: App missing from menu; fails to launch; file browser
  fails to open or list files; crashes or freezes on either the normal or
  invalid-file case; Back does not return cleanly; **any of
  `cat.bm`/`dolphin.bm`/`spongebob.bm` found anywhere on the SD card**;
  **any new or modified file appears on the SD card after using this
  app**; any global fail condition from the plan.
- **Notes field**: _____________________________________________

---

## 15. `boilerplate` / `fap_boilerplate` (FAP Boilerplate)

- **Category / menu path**: Apps → Tools → "FAP Boilerplate" (manifest
  `appid="fap_boilerplate"`, not `boilerplate` — see
  `docs/PHASE2E_2_LICENSE_ATTRIBUTION.md` for the discrepancy note)
- **Launch path**: From the Tools submenu, select "FAP Boilerplate" and
  press OK.
- **Expected screen on launch**: The app's own demonstration/template
  start screen (this is an example/template app, so the exact screen is
  whatever the author's own template scenes show — per
  `docs/PHASE2E_2_LICENSE_ATTRIBUTION.md`, the tree includes
  `helpers/`, `scenes/`, and `views/`, consistent with a multi-scene
  demonstration app).
- **Minimum navigation test**: From the start screen, navigate through
  whatever menu/scene options the app exposes and confirm each renders
  without error.
- **One normal input test**: Exercise the app's primary demonstrated
  function (whatever its own template scene provides — e.g. a settings or
  demo action) and confirm it completes without error.
- **One invalid/edge input test**: Press an arbitrary D-pad direction or
  button the current scene does not define a handler for, and confirm it
  is silently ignored or handled gracefully rather than crashing.
- **Exit behavior**: Press Back repeatedly from any scene until returning
  to the Tools menu; confirm this happens cleanly.
- **SD-card inspection (specific to this app — required, not optional)**:
  After exercising the app's own functionality, confirm any file written
  appears **only** under `/ext/apps_data/boilerplate/` and nowhere else on
  the SD card — source confirms this app writes its own configuration
  file to that path (`docs/PHASE2E_1_SOURCE_LICENSE_VERIFICATION.md`,
  `docs/PHASE2E_2_LICENSE_ATTRIBUTION.md`). This step exists to confirm
  that holds on real hardware, not just in source.
- **Expected app-private storage path**: `/ext/apps_data/boilerplate/`
  only. Any write outside this path is a fail condition per the plan.
- **Pass criteria**: App appears in Tools menu; launches; scene navigation
  works without error; primary demonstrated function completes without
  error; unhandled input is ignored gracefully; Back returns cleanly to
  the menu; any write is confined to `/ext/apps_data/boilerplate/`.
- **Fail criteria**: App missing from menu; fails to launch; crashes or
  freezes on any scene/input case; Back does not return to the menu; any
  write outside `/ext/apps_data/boilerplate/`; any global fail condition
  from the plan.
- **Notes field**: _____________________________________________

---

## 16. `minesweeper` / `minesweeper_redux` (Minesweeper Redux)

- **Category / menu path**: Apps → Games → "Minesweeper Redux" (manifest
  `appid="minesweeper_redux"`, not `minesweeper` — see
  `docs/PHASE2E_2_LICENSE_ATTRIBUTION.md` for the discrepancy note)
- **Launch path**: From the Games submenu, select "Minesweeper Redux" and
  press OK.
- **Expected screen on launch**: A start-screen animation (per
  `docs/PHASE2E_THIRD_PARTY_NOTICES.md`, a 13-frame original start-screen
  animation), leading into a rendered minefield grid with a movable
  cursor.
- **Minimum navigation test**: Use the D-pad to move the cursor across
  several cells on the minefield, confirming the highlighted/selected
  cell changes accordingly and the cursor does not move off the board or
  get stuck.
- **One normal input test**: Move the cursor to an unrevealed cell and
  press OK to reveal it, and confirm the board updates to show either a
  number, a blank/cascade reveal, or a mine, per standard Minesweeper
  rules.
- **One invalid/edge input test**: Flag a cell (via whichever button the
  app documents for flagging) and confirm the flag indicator displays
  correctly; separately, reveal a mine deliberately (or play until one is
  revealed) and confirm the app shows a clear game-over
  ("crying dolphin" game-over sprite, per
  `docs/PHASE2E_THIRD_PARTY_NOTICES.md`) state rather than crashing or
  freezing.
- **Save/load behavior (specific to this app)**: Confirmed in source
  (`docs/PHASE2E_1_SOURCE_LICENSE_VERIFICATION.md`,
  `docs/PHASE2E_2_LICENSE_ATTRIBUTION.md`) to save/load state under
  `/ext/apps_data/mine_sweeper_redux/` via an atomic write-then-rename
  pattern. Exit the game (or trigger its own save behavior) and confirm
  the app resumes or restarts consistently on next launch without error
  or corruption, and that any save/config file is written only under
  `/ext/apps_data/mine_sweeper_redux/` and nowhere else on the SD card —
  including confirming no orphaned temp file from an interrupted
  write-then-rename survives outside that directory.
- **Exit behavior**: Press Back from the game-over screen (and separately
  from the board mid-game) until returning to the Games menu; confirm
  this happens cleanly.
- **Expected app-private storage path**: `/ext/apps_data/mine_sweeper_redux/`
  only. Any write outside this path is a fail condition per the plan.
- **Pass criteria**: App appears in Games menu; launches; start animation
  and board render; cursor movement, cell reveal, and flagging all work;
  revealing a mine triggers a clean game-over state (no crash/freeze);
  save/load (if exercised) completes without error and only touches
  `/ext/apps_data/mine_sweeper_redux/`; Back returns cleanly to the Games
  menu.
- **Fail criteria**: App missing from Games menu; fails to launch; crashes
  or freezes on any input case (including on mine reveal); save/load
  produces an error, corrupts data, or writes outside
  `/ext/apps_data/mine_sweeper_redux/`; Back does not return to the menu;
  any global fail condition from the plan.
- **Notes field**: _____________________________________________
...
- **Category / menu path**: Apps → Tools → "QR Code"
- **Launch path**: From the Tools submenu, select "QR Code" and press OK.
- **Expected screen on launch**: A menu/list screen (per source,
  `qrcode_app.c`) offering existing saved QR entries (if any) plus an
  option to create a new one; on first-ever launch this list may be
  empty.
- **Minimum navigation test**: From the list screen, navigate into "add
  new" / create flow, then back out via Back without saving, confirming
  Back returns to the list screen rather than exiting the app entirely on
  the first press.
- **One normal input test**: Create a new QR entry by entering a short
  text payload (e.g. a plain URL such as `https://example.com`) via the
  on-screen keyboard, confirm, and confirm a rendered QR code is
  displayed on screen for that payload.
- **One invalid/edge input test**: Attempt to create an entry with an
  empty payload (confirm with no text entered), or a payload at/near the
  app's maximum supported length, and confirm the app either rejects the
  attempt with an on-screen message or handles the boundary case
  gracefully, without crashing or freezing.
- **Exit behavior**: Press Back from the QR-display screen to return to
  the list, and again from the list to return to the Tools menu; confirm
  both happen cleanly.
- **SD-card / legacy-migration inspection (specific to this app)**: Before
  first launch, note whether a legacy `/ext/qrcodes/` folder exists on the
  test SD card. If it does, confirm after launch that its contents were
  migrated into the app's own `QRCODE_FOLDER` and the legacy folder itself
  was removed (`storage_common_copy` + `storage_common_remove`, per
  source) — a one-time, bounded, expected migration, not a defect. If no
  legacy folder exists (the common case on a fresh SD card), confirm no
  write of any kind occurs anywhere on the SD card while only viewing
  and/or generating QR codes, since QR content itself is read-only by
  source (`qrcode_load_file()` uses `flipper_format_read_*` exclusively —
  see `qrcodeStorageNote` in `tools/phase2f_hardware_gate_config.json`
  and `docs/PHASE2F_2_SAFETY_REVIEW.md`).
- **Expected app-private storage path**: No documented app-private save
  path beyond whatever pre-existing file the user's own saved QR content
  lives in; this checklist step exists specifically to confirm no
  unexpected write occurs beyond the one-time legacy migration described
  above.
- **Pass criteria**: App appears in Tools menu; launches; list screen
  renders; a new QR entry can be created from valid text and displays a
  rendered QR code; empty/oversized payload is handled without
  crash/freeze; Back returns cleanly through both screens; the only
  storage write observed (if any) is the one-time legacy `/ext/qrcodes/`
  migration described above — no other write occurs.
- **Fail criteria**: App missing from menu; fails to launch; list screen
  or QR-render screen fails to display; crashes or freezes on either input
  case; Back does not return cleanly; **any storage write other than the
  documented one-time legacy migration**; any global fail condition from
  the plan.
- **Notes field**: _____________________________________________

---

## 18. `hex_viewer` (HEX Viewer)

- **Category / menu path**: Apps → Tools → "HEX Viewer"
- **Launch path**: From the Tools submenu, select "HEX Viewer" and press
  OK.
- **Expected screen on launch**: A file-browser dialog (the standard
  Flipper file-browser widget) prompting the user to select a file to
  view in hex.
- **Minimum navigation test**: Navigate the file browser to any file
  present on the SD card and confirm selecting it opens the hex-view
  screen, rendering the file's byte contents in hex/ASCII columns.
- **One normal input test**: With a file open, use Up/Down (or
  documented scroll controls) to scroll through the hex view, and confirm
  the displayed offset/bytes change accordingly.
- **One invalid/edge input test**: Open the app's own Settings scene
  (haptic/speaker/LED/save-settings toggles, per source) and toggle one
  setting, confirming the toggle visibly updates; separately, attempt to
  scroll past the end of a very small file and confirm the view stops
  cleanly rather than crashing or freezing.
- **Exit behavior**: Press Back from the hex-view screen to return to the
  file browser, and again to return to the Tools menu; confirm both
  happen cleanly.
- **SD-card inspection (specific to this app — required, not optional)**:
  After viewing one or more files and toggling a setting, confirm **the
  viewed file itself is never modified** — source confirms files are
  opened with `FSAM_READ`/`FSOM_OPEN_EXISTING` only, with no write, edit,
  patch, overwrite, or delete call touching the viewed file anywhere in
  the source (`docs/PHASE2F_2_SAFETY_REVIEW.md`). Separately, confirm any
  settings write is confined to `/ext/apps_data/hex_viewer/` and nowhere
  else on the SD card.
- **Expected app-private storage path**: `/ext/apps_data/hex_viewer/`
  only, for this app's own 4 boolean settings toggles. The viewed file
  itself must never be written to under any circumstance.
- **Pass criteria**: App appears in Tools menu; launches; file browser
  opens and a file can be selected and viewed in hex; scrolling works;
  settings toggle works and is confined to
  `/ext/apps_data/hex_viewer/`; small-file boundary scroll is handled
  without crash/freeze; Back returns cleanly through both screens; **the
  viewed file is never modified**.
- **Fail criteria**: App missing from menu; fails to launch; file browser
  fails to open or list files; crashes or freezes on any case above; Back
  does not return cleanly; **the viewed file is modified in any way**;
  **any settings write occurs outside `/ext/apps_data/hex_viewer/`**; any
  global fail condition from the plan.
- **Notes field**: _____________________________________________

---

## 19. `barcode_gen` (Barcode App)

- **Category / menu path**: Apps → Tools → "Barcode App" (manifest
  `appid="barcode_app"`, not `barcode_gen` — the directory name and the
  manifest appid differ, matching this project's established pattern for
  `boilerplate`/`fap_boilerplate` and `minesweeper`/`minesweeper_redux`)
- **Launch path**: From the Tools submenu, select "Barcode App" and press
  OK.
- **Expected screen on launch**: A list screen offering existing saved
  barcode entries (if any, under `/ext/apps_data/barcodes/`) plus an
  option to create a new one; on first-ever launch this list may be
  empty.
- **Minimum navigation test**: From the list screen, navigate into the
  create/new-entry flow, then back out via Back without saving, confirming
  Back returns to the list screen rather than exiting the app entirely on
  the first press. **This is also the specific screen where the Phase
  2F.2A source fix applies** (`views/create_view.c`, dead calls to
  `text_input_show_illegal_symbols` removed, commit `b6445ed`) — confirm
  the on-screen keyboard/text-entry widget used here is the standard
  system `TextInput` (no crash or undefined-behavior symptom specific to
  this screen).
- **One normal input test**: Create a new barcode entry by entering a
  short, valid name/value via the on-screen keyboard, confirm, and confirm
  a rendered barcode is displayed on screen for that entry.
- **One invalid/edge input test**: Attempt to create an entry with an
  empty name, or a name containing characters the app's own validator
  rejects (per `barcode_validator.c`'s error-classification enum,
  e.g. `MissingEncodingTable`/`EncodingTableError` conditions), and
  confirm the app shows a clear on-screen error/refusal rather than
  crashing or freezing; separately, delete an existing entry (if one was
  created) and confirm deletion completes without error.
- **Exit behavior**: Press Back from the barcode-display screen to return
  to the list, and again from the list to return to the Tools menu;
  confirm both happen cleanly.
- **SD-card inspection (specific to this app — required, not optional)**:
  After creating, viewing, and deleting one or more barcode entries,
  confirm any file written (create/edit/rename/delete) is confined to
  `/ext/apps_data/barcodes/` and nowhere else on the SD card — source
  confirms all such operations (`storage_simply_mkdir`,
  `storage_common_rename`, `storage_simply_remove`) operate only on paths
  constructed from `DEFAULT_USER_BARCODES`
  (`docs/PHASE2F_2_SAFETY_REVIEW.md`). Also confirm no radio/RF/hardware
  activity of any kind is observed while using this app — it performs
  local barcode display/generation only, with no scanner-emulation,
  access-control, HID, NFC, or USB behavior.
- **Expected app-private storage path**: `/ext/apps_data/barcodes/` only.
  Any write outside this path is a fail condition per the plan.
- **Pass criteria**: App appears in Tools menu; launches; list screen
  renders; a new barcode entry can be created from a valid name and
  displays a rendered barcode; invalid name is handled without
  crash/freeze; deletion (if exercised) completes without error; the
  create/text-entry screen (Phase 2F.2A's fix location) shows no crash or
  keyboard-related defect; Back returns cleanly through both screens; any
  write is confined to `/ext/apps_data/barcodes/`; no radio/RF/hardware
  activity observed.
- **Fail criteria**: App missing from menu; fails to launch; list screen
  or barcode-render screen fails to display; crashes or freezes on any
  case above (especially on the create/text-entry screen); deletion
  produces an error or corrupts data; Back does not return cleanly; **any
  write outside `/ext/apps_data/barcodes/`**; **any radio/RF/hardware
  activity observed**; any global fail condition from the plan.
- **Notes field**: _____________________________________________

---

## 20. `fcc_id_lookup` (FCC ID Lookup)

- **Category / menu path**: Apps → Tools → "FCC ID Lookup"
- **Launch path**: From the Tools submenu, select "FCC ID Lookup" and
  press OK.
- **Expected screen on launch**: Depends on whether the optional FCC
  frequency/applicant database has been separately, manually placed at
  `/ext/apps_assets/fcc_id_lookup/fcc_freq_v2.bin` by the tester. On
  this accepted baseline, that database is **not bundled and not
  expected to be present** — the expected screen on a fresh device is
  the app's own setup-hint message (source: `FCC_DB_SETUP_HINT`,
  "Copy BIN file into /ext/apps_assets/fcc_id_lookup to install the
  database.") rather than a crash or blank screen. If a tester has
  manually installed the optional database, the expected screen is
  instead an FCC ID entry/search prompt.
- **Minimum navigation test**: From the setup-hint (or entry) screen,
  navigate into any available submenu (e.g. an "about"/attribution
  screen, if present) and back out via Back, confirming Back does not
  exit past the app unexpectedly on the first press.
- **One normal input test**: If the optional database is present,
  enter a well-formed FCC ID (e.g. an example from the app's own
  README) and confirm a lookup result or a "not found in database"
  message is displayed without crashing. If the optional database is
  absent (the expected state for this baseline), confirm instead that
  attempting to proceed past the setup-hint screen produces a graceful
  on-screen message rather than a crash.
- **One invalid/edge input test**: Enter a malformed or empty FCC ID
  (if the entry screen is reachable) and confirm the app rejects it
  with an on-screen message rather than crashing or freezing.
- **Exit behavior**: Press Back repeatedly from wherever the
  navigation above ends until returning to the main Tools menu;
  confirm this happens cleanly (no crash, no stuck screen).
- **SD-card inspection (specific to this app — required, not
  optional)**: After using the app, confirm **no file of any kind is
  written to the SD card** — source confirms zero write-capable storage
  calls anywhere (all 11 storage call sites are read/open/close/free/
  seek/size operations only, `docs/FCC_ID_LOOKUP_SAFETY_REVIEW.md`).
  Also confirm the app's only optional data access
  (`/ext/apps_assets/fcc_id_lookup/fcc_freq_v2.bin`) is read-only, and
  that no `.bin` file was created by the app itself (only a
  human-placed one, if the tester chose to install the optional
  database separately — this app must never create that file on its
  own).
- **Expected app-private storage path**: None for writes (zero
  write-capable storage calls). Optional read-only access at
  `/ext/apps_assets/fcc_id_lookup/fcc_freq_v2.bin`, which this project
  never bundles or creates.
- **Pass criteria**: App appears in Tools menu; launches; setup-hint
  message displays correctly when the optional database is absent (the
  expected state for this baseline) rather than crashing; navigation
  and Back behavior work cleanly; invalid input (if the entry screen is
  reachable) is handled without crash/freeze; **no file of any kind is
  written to the SD card**.
- **Fail criteria**: App missing from menu; fails to launch; crashes or
  shows a blank/frozen screen instead of the setup-hint message when the
  database is absent; crashes or freezes on any input case; Back
  navigation does not return cleanly; **any file is written to the SD
  card by this app**; any global fail condition from the plan.
- **Notes field**: _____________________________________________

---

## After completing all 20 apps

- Confirm no global fail condition occurred at any point across all 19
  apps.
- Confirm specifically that `sd_info` left no `/ext/sdtest.tmp*` files
  ...
  created/modified no file at all, that `2048` wrote only to
  `/ext/apps_data/game_2048/`, that `image_viewer` created/modified no
  file at all AND that `cat.bm`/`dolphin.bm`/`spongebob.bm` do not exist
  anywhere on the SD card, that `boilerplate` wrote only to
  `/ext/apps_data/boilerplate/`, that `minesweeper` wrote only to
  `/ext/apps_data/mine_sweeper_redux/`, that `qrcode` performed no write
  beyond the documented one-time legacy `/ext/qrcodes/` migration (if
  applicable), that `hex_viewer` never modified any viewed file and wrote
  only to `/ext/apps_data/hex_viewer/`, and that `barcode_gen` wrote only
  to `/ext/apps_data/barcodes/`, and that `fcc_id_lookup` wrote no file
  of any kind to the SD card — these are the twelve storage-behavior
  confirmations most worth double-checking in this final baseline.
