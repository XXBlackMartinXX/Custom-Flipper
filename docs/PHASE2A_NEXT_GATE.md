...
| Artifact hash finalization (Phase 2A.10) | **Finalized.** Real SHA-256 hashes of `firmware.dfu` and the updater `.tgz` were generated by the `Phase 2A Finalize Baseline` GitHub Actions workflow (run [`28859929957`](https://github.com/XXBlackMartinXX/Custom-Flipper/actions/runs/28859929957)), which downloads its own source run's artifacts directly and hashes them on a real Windows runner - see `docs/PHASE2A_ARTIFACT_HASHES.md`. |
| Hardware-assisted validation gate (Phase 2A.12) | **Built and exercised, classification `HARDWARE VALIDATION BLOCKED - DEVICE NOT AVAILABLE`.** `tools/phase2a_hardware_gate.ps1` exists and was run for real in the AI session's own (non-Windows, deviceless) sandbox; see `docs/PHASE2A_HARDWARE_ASSISTED_RESULTS.md` for the full per-mode results. No physical Flipper Zero and no Windows machine were available to complete device-connected or GUI-level checks. |
...
running `tools/phase2a_hardware_gate.ps1 -Mode HardwareAssisted
-ArtifactDir <path>` (Phase 2A.12; branch/commit check, real artifact hash
verification, device detection, qFlipper detection, and — only after
explicit confirmation — the flash-confirmation gate, never an automatic
flash) and, separately, walking through
`docs/PHASE2A_HARDWARE_SMOKE_TEST_CHECKLIST.md` on the real device,
recording results in a filled-in copy of
...
**Path A is now active** (Phase 2A.12 tooling and docs exist), but has only
been exercised in the AI session's own sandbox, where it correctly and
honestly classified as `HARDWARE VALIDATION BLOCKED - DEVICE NOT
AVAILABLE` (no Windows machine, no physical device — see
`docs/PHASE2A_HARDWARE_ASSISTED_RESULTS.md`). **Phase 2B planning (Path B)
remains blocked until Phase 2A.12 is classified by an actual run against
real hardware or a deliberate decision to proceed without one**, per the
following rule:

- If a real Phase 2A.12 run reaches **PASS** or **PASS WITH HUMAN
  OBSERVATION** (i.e. hardware-connected checks pass and the human-observed
  GUI checklist is completed), Phase 2B planning (Path B) may start.
- If Phase 2A.12 is **BLOCKED** (as it currently is, in this sandbox — no
  device available), Phase 2B planning may still start, but **only** if it
  ...
  skip, actual hardware testing, and never as authorization to import code.
- If Phase 2A.12 **FAILS** (e.g. an artifact hash mismatch, or a real
  device/GUI check finding a defect), Phase 2B planning and import must not
  start until the failure is root-caused and resolved.

Given the current classification (`HARDWARE VALIDATION BLOCKED - DEVICE NOT
AVAILABLE`), Phase 2B planning may proceed **only** as clearly-labeled
non-hardware-dependent planning, and only once the project owner explicitly
requests it — the same standing rule as Path B below, unchanged.
