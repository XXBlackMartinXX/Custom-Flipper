# Phase 2C — Hardware-Assisted Validation

Docs only. This is the Phase 2C counterpart to
`docs/PHASE2A_HARDWARE_ASSISTED_VALIDATION.md` and
`docs/PHASE2B_HARDWARE_ASSISTED_VALIDATION.md` — same discipline, extended
to cover all 10 apps in the accepted Phase 2C baseline. `tools/phase2c_hardware_gate.ps1`
and `tools/phase2c_hardware_gate_config.json` implement this gate;
`docs/PHASE2C_HARDWARE_ASSISTED_RESULTS.md` records what actually
happened when it was run.

## What this phase validates

- **Repository/branch/commit state** against the accepted Phase 2C
  baseline (`integration/phase2c-first-batch` at
  `969054ee9f802f72be1064a62052c4be82a91783`).
- **Artifact integrity** — if a downloaded copy of `firmware.dfu` and the
  ...
  `docs/PHASE2C_3_ARTIFACT_HASHES.md`.
...
- **It does not confirm the firmware boots correctly**, that any app
  ...
  checklist's job (`docs/PHASE2C_HARDWARE_SMOKE_TEST_CHECKLIST.md`), not
  this script's.
...
| Accepted CI baseline commit | `969054ee9f802f72be1064a62052c4be82a91783` |
| CI validation run | [`28897702247`](https://github.com/XXBlackMartinXX/Custom-Flipper/actions/runs/28897702247) |
| Finalization run | [`28899393035`](https://github.com/XXBlackMartinXX/Custom-Flipper/actions/runs/28899393035) |
...
| Apps covered (10) | `network_subnet`, `programmer_calc`, `vin_decoder`, `flipper95`, `chess`, `flipfetch`, `quadratic_solver`, `sudoku`, `sd_info`, `docviewlite` |
...
implemented anywhere in `tools/phase2c_hardware_gate.ps1`.
...
the flash-confirmation gate. This logic was verified in this phase with a
deliberate synthetic-mismatch test (two random-data files of the exact
expected sizes) — see `docs/PHASE2C_HARDWARE_ASSISTED_RESULTS.md`.
...
batch-agnostic and applies to the Phase 2C-inclusive baseline exactly as
it did for Phase 2A and Phase 2B.
...
enumerates one `REQUIRES_HUMAN_OBSERVATION` entry per app (all 10), plus
dedicated entries for `chess`'s and `sudoku`'s save/load private-path
checks, `sd_info`'s SD-benchmark temp-file cleanup check, and
`docviewlite`'s read-only confirmation check — never silently omitted,
never simulated. See `docs/PHASE2C_HARDWARE_SMOKE_TEST_CHECKLIST.md` for
the actual human-performed steps.

## How to interpret PASS / FAIL / BLOCKED / NEEDS REVIEW
...
| `HARDWARE VALIDATION PASS WITH HUMAN OBSERVATION PENDING` | Device detected, automated checks clean, but GUI-level checks remain outstanding — completing `docs/PHASE2C_HARDWARE_SMOKE_TEST_CHECKLIST.md` on the real device is still required before any release-readiness claim. |
...
because GUI-level behavior is never automatable.
...
release-gate checklist beyond Phase 2C alone. Nothing in this document or
in `tools/phase2c_hardware_gate.ps1` itself ever constitutes that
acceptance.
