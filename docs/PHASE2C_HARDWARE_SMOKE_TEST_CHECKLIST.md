# Phase 2C — Hardware Smoke Test Checklist

Docs only. This is the Phase 2C counterpart to
`docs/PHASE2B_HARDWARE_SMOKE_TEST_CHECKLIST.md` (which remains unmodified
and still valid on its own for the 8 apps it covers) — read
...
**Phase 2C.4 cross-reference**: `tools/phase2c_hardware_gate.ps1 -Mode
...
and the flash-confirmation gate) and lists every one of the 10 apps below
...
`docs/PHASE2C_HARDWARE_ASSISTED_VALIDATION.md` for what that gate does and
does not validate.

Menu path prefix for all 10 apps: **Main Menu → Apps → \<category\>**
(category noted per app below), then the app's own name.

Sections 1–8 (`network_subnet`, `programmer_calc`, `vin_decoder`,
`flipper95`, `chess`, `flipfetch`, `quadratic_solver`, `sudoku`) are
identical in content to `docs/PHASE2B_HARDWARE_SMOKE_TEST_CHECKLIST.md` —
reproduced here so this document is a complete, standalone checklist for
the full 10-app Phase 2C baseline, not a fragment. Sections 9–10 are new,
for the 2 Phase 2C apps. `fcc_id_lookup` is **not** included — it is not
part of the accepted Phase 2C baseline (deferred pending a license-evidence
resolution).
...
## After completing all 10 apps

- Confirm no global fail condition occurred at any point across all 10
  apps.
- Confirm specifically that `sd_info` left no `/ext/sdtest.tmp*` files
  behind and that `docviewlite` created/modified no file at all — these
  are the two storage-behavior confirmations unique to this batch.
