# Phase 2E — Hardware-Assisted Validation
...
`docs/PHASE2C_HARDWARE_ASSISTED_VALIDATION.md`, and
`docs/PHASE2D_HARDWARE_ASSISTED_VALIDATION.md` — same discipline, extended
to cover all 16 apps in the accepted Phase 2E baseline.
`tools/phase2e_hardware_gate.ps1` and
`tools/phase2e_hardware_gate_config.json` implement this gate;
`docs/PHASE2E_HARDWARE_ASSISTED_RESULTS.md` records what actually
happened when it was run.

## What this phase validates

- **Repository/branch/commit state** against the accepted Phase 2E
  baseline (`integration/phase2e-first-batch` at
  `dcdfbb4c262c585d7d4126dc21b40dc3b948fc93`).
...
- **Artifact integrity** — if a downloaded copy of `firmware.dfu` and the
  ...
  `docs/PHASE2E_3_ARTIFACT_HASHES.md`.
...
- **It does not confirm the firmware boots correctly**, that any app
  ...
  checklist's job (`docs/PHASE2E_HARDWARE_SMOKE_TEST_CHECKLIST.md`), not
  this script's.
...
| Accepted CI baseline commit | `dcdfbb4c262c585d7d4126dc21b40dc3b948fc93` |
| Final integration commit | `f2340ae` |
| CI validation run | [`28968511276`](https://github.com/XXBlackMartinXX/Custom-Flipper/actions/runs/28968511276) |
| Finalization run | [`28972160432`](https://github.com/XXBlackMartinXX/Custom-Flipper/actions/runs/28972160432) |
...
| Apps covered (16) | `network_subnet`, `programmer_calc`, `vin_decoder`, `flipper95`, `chess`, `flipfetch`, `quadratic_solver`, `sudoku`, `sd_info`, `docviewlite`, `resistors`, `crypto_dictionary`, `2048`, `image_viewer`, `boilerplate`/`fap_boilerplate`, `minesweeper`/`minesweeper_redux` |
...
implemented anywhere in `tools/phase2e_hardware_gate.ps1`.
...
`docs/PHASE2E_HARDWARE_ASSISTED_RESULTS.md` for whether that test was
...
batch-agnostic and applies to the Phase 2E-inclusive baseline exactly as
it did for Phase 2A, Phase 2B, Phase 2C, and Phase 2D.
...
enumerates one `REQUIRES_HUMAN_OBSERVATION` entry per app (all 16), plus
...
on the device's own file browser), and `boilerplate`'s and
`minesweeper`'s app-private storage-path checks — never silently
omitted, never simulated. See
`docs/PHASE2E_HARDWARE_SMOKE_TEST_CHECKLIST.md` for the actual
...
| `HARDWARE VALIDATION FAILED` | A real check failed — e.g. an artifact hash mismatch, or the excluded `example_images/` directory reappearing. Do not proceed to a flash. |
| `HARDWARE VALIDATION PASS WITH HUMAN OBSERVATION PENDING` | Device detected, automated checks clean, but GUI-level checks remain outstanding — completing `docs/PHASE2E_HARDWARE_SMOKE_TEST_CHECKLIST.md` on the real device is still required before any release-readiness claim. |
...
`docs/PHASE2E_HARDWARE_SMOKE_TEST_CHECKLIST.md` on a real device and
...
presence, tooling presence, excluded-asset absence) are satisfied — it
does not mean any app has actually been launched and observed on a real
screen, that no crash/reboot/freeze occurred, or that storage behavior
matches expectations. Release-readiness requires the human-performed
smoke-test checklist to actually be completed and its results recorded,
plus this project's full release-gate checklist beyond Phase 2E alone.
Nothing in this document or in `tools/phase2e_hardware_gate.ps1` itself
