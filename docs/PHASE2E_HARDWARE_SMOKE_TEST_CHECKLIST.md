# Phase 2E — Hardware Smoke Test Checklist

Docs only. This is the Phase 2E counterpart to
`docs/PHASE2D_HARDWARE_SMOKE_TEST_CHECKLIST.md` (which remains unmodified
and still valid on its own for the 13 apps it covers) — read
...
**Phase 2E.4 cross-reference**: `tools/phase2e_hardware_gate.ps1 -Mode
...
excluded-asset absence check, and the flash-confirmation gate) and lists
every one of the 16 apps below as `REQUIRES_HUMAN_OBSERVATION`, pointing
back to this checklist — it does not and cannot perform any of the steps
in this document itself. See
`docs/PHASE2E_HARDWARE_ASSISTED_VALIDATION.md` for what that gate does and
does not validate.
...
anywhere in this checklist — none of the 16 apps in this batch requires
them, and this project's own safety-exclusion policy forbids adding them
here regardless.

Menu path prefix for all 16 apps: **Main Menu → Apps → \<category\>**
(category noted per app below), then the app's own name.

Sections 1–13 (`network_subnet`, `programmer_calc`, `vin_decoder`,
`flipper95`, `chess`, `flipfetch`, `quadratic_solver`, `sudoku`,
`sd_info`, `docviewlite`, `resistors`, `crypto_dictionary`, `2048`) are
identical in content to `docs/PHASE2D_HARDWARE_SMOKE_TEST_CHECKLIST.md` —
reproduced here so this document is a complete, standalone checklist for
the full 16-app Phase 2E baseline, not a fragment. Sections 14–16 are new,
for the 3 Phase 2E apps. `fcc_id_lookup` is **not** included — it is not
part of the accepted Phase 2E baseline (deferred pending a
license-evidence resolution).
...
## After completing all 16 apps

- Confirm no global fail condition occurred at any point across all 16
  apps.
- Confirm specifically that `sd_info` left no `/ext/sdtest.tmp*` files
  ...
  `/ext/apps_data/boilerplate/`, and that `minesweeper` wrote only to
  `/ext/apps_data/mine_sweeper_redux/` — these are the eight
  storage-behavior confirmations most worth double-checking in this
  batch.
