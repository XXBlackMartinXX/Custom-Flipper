# Phase 2F — Hardware Smoke Test Checklist

Docs only. This is the Phase 2F counterpart to
`docs/PHASE2E_HARDWARE_SMOKE_TEST_CHECKLIST.md` (which remains unmodified
and still valid on its own for the 16 apps it covers) — read
...
**Phase 2F.4 cross-reference**: `tools/phase2f_hardware_gate.ps1 -Mode
...
excluded-asset absence check, `barcode_gen` source-fix preservation
check, and the flash-confirmation gate) and lists every one of the 19
apps below as `REQUIRES_HUMAN_OBSERVATION`, pointing back to this
checklist — it does not and cannot perform any of the steps in this
document itself. See `docs/PHASE2F_HARDWARE_ASSISTED_VALIDATION.md` for
what that gate does and does not validate.
...
anywhere in this checklist — none of the 19 apps in this batch requires
them, and this project's own safety-exclusion policy forbids adding them
here regardless.

Menu path prefix for all 19 apps: **Main Menu → Apps → \<category\>**
(category noted per app below), then the app's own name.

Sections 1–16 (`network_subnet`, `programmer_calc`, `vin_decoder`,
...
`minesweeper`/`minesweeper_redux`) are identical in content to
`docs/PHASE2E_HARDWARE_SMOKE_TEST_CHECKLIST.md` — reproduced here so this
document is a complete, standalone checklist for the full 19-app Phase 2F
baseline, not a fragment. Sections 17–19 are new, for the 3 Phase 2F
apps (`qrcode`, `hex_viewer`, `barcode_gen`). `fcc_id_lookup` is **not**
included — it is not part of the accepted Phase 2F baseline (deferred
pending a license-evidence resolution).
...
- Confirm specifically that `sd_info` left no `/ext/sdtest.tmp*` files
  ...
  to `/ext/apps_data/barcodes/` — these are the eleven storage-behavior
