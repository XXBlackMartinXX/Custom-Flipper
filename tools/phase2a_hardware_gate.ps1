<#
.SYNOPSIS
    Phase 2A.12 hardware-assisted validation gate for the accepted Phase 2A
    CI baseline (Custom-Flipper, integration/phase2a-first-batch).
...
.DESCRIPTION
    ...
    even in -Mode HardwareAssisted with -AllowFlashPrompt.

    It validates the Phase 2A baseline recorded in
    tools/phase2a_hardware_gate_config.json (branch, commit, and the real,
    GitHub-Actions-generated artifact hashes from docs/PHASE2A_ARTIFACT_HASHES.md)
...
.PARAMETER Mode
    ...
    HardwareAssisted    - Combines DetectDevice + HashVerify (if -ArtifactDir
                           ...
                           docs/PHASE2A_HARDWARE_SMOKE_TEST_CHECKLIST.md.
...
.PARAMETER ArtifactDir
    Path to an already-downloaded-and-extracted Phase 2A CI artifact
...
.PARAMETER ReportDir
    ...
    reports\phase2a_hardware under the repo root. Created if missing.
...
.EXAMPLE
    .\tools\phase2a_hardware_gate.ps1 -Mode Preflight

.EXAMPLE
    .\tools\phase2a_hardware_gate.ps1 -Mode HashVerify -ArtifactDir "C:\Downloads\phase2a-firmware-artifacts"

.EXAMPLE
    .\tools\phase2a_hardware_gate.ps1 -Mode DetectDevice

.EXAMPLE
    .\tools\phase2a_hardware_gate.ps1 -Mode HardwareAssisted -ArtifactDir "C:\Downloads\phase2a-firmware-artifacts" -AllowFlashPrompt
#>
...
if (-not $ReportDir) {
    $ReportDir = Join-Path $RepoRoot 'reports\phase2a_hardware'
}
if (-not (Test-Path $ReportDir)) {
    New-Item -ItemType Directory -Path $ReportDir -Force | Out-Null
}

$ConfigPath = Join-Path $ScriptRoot 'phase2a_hardware_gate_config.json'
if (-not (Test-Path $ConfigPath)) {
    throw "Config file not found at '$ConfigPath'. This script requires tools\phase2a_hardware_gate_config.json to exist alongside it."
}
...
Write-Host ''
Write-Host '=== Phase 2A.12 Hardware-Assisted Validation Gate ===' -ForegroundColor Cyan
...
Write-Host 'app behavior always requires a human running docs/PHASE2A_HARDWARE_SMOKE_TEST_CHECKLIST.md.' -ForegroundColor Yellow
...
$reportObject = [ordered]@{
    ...
    nextActions    = @(
        ...
        'REQUIRES_HUMAN_OBSERVATION entries are not optional to skip - complete docs/PHASE2A_HARDWARE_SMOKE_TEST_CHECKLIST.md on the real device and record results in a copy of docs/PHASE2A_HARDWARE_TEST_RESULTS_TEMPLATE.md.',
        'This report alone never constitutes hardware-tested or release-ready status.'
    )
}

$jsonPath = Join-Path $ReportDir "phase2a_hardware_gate_$RunTimestampForFilename.json"
$reportObject | ConvertTo-Json -Depth 10 | Set-Content -Path $jsonPath -Encoding UTF8

$md = New-Object System.Collections.Generic.List[string]
$md.Add('# Phase 2A.12 Hardware-Assisted Validation Gate Report') | Out-Null
$md.Add('') | Out-Null
...
$mdPath = Join-Path $ReportDir "phase2a_hardware_gate_$RunTimestampForFilename.md"
