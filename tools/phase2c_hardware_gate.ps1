<#
.SYNOPSIS
    Phase 2C.4 hardware-assisted validation gate for the accepted Phase 2C
    CI baseline (Custom-Flipper, integration/phase2c-first-batch).
...
.DESCRIPTION
    ...
    2C counterpart to tools/phase2a_hardware_gate.ps1 and
    ...
    extended to cover all 10 apps now in the accepted Phase 2C baseline.
    ...
    It validates the Phase 2C baseline recorded in
    tools/phase2c_hardware_gate_config.json (branch, commit, and the real,
    GitHub-Actions-generated artifact hashes from docs/PHASE2C_3_ARTIFACT_HASHES.md)
    ...
    Report-filename collision fix (new in Phase 2C.4): Phase 2A's and Phase
    2B's hardware gate scripts used second-granularity report filenames, so
    two invocations within the same wall-clock second could silently
    overwrite each other's report. This script uses a millisecond-precision
    timestamp plus a short random hex suffix instead
    (yyyyMMdd_HHmmss_fff_XXXX), which is collision-resistant even for rapid
    repeated invocations. Phase 2A's and Phase 2B's own scripts are
...
.PARAMETER Mode
    ...
    HardwareAssisted    - Combines DetectDevice + HashVerify (if -ArtifactDir
                           ...
                           docs/PHASE2C_HARDWARE_SMOKE_TEST_CHECKLIST.md.
...
.PARAMETER ArtifactDir
    Path to an already-downloaded-and-extracted Phase 2C CI artifact
...
.PARAMETER ReportDir
    ...
    reports\phase2c_hardware under the repo root. Created if missing.
...
.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\tools\phase2c_hardware_gate.ps1 -Mode Preflight

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\tools\phase2c_hardware_gate.ps1 -Mode HashVerify -ArtifactDir "<artifact-dir>"

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\tools\phase2c_hardware_gate.ps1 -Mode DetectDevice

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\tools\phase2c_hardware_gate.ps1 -Mode HardwareAssisted -ArtifactDir "<artifact-dir>" -AllowFlashPrompt
#>
...
if (-not $ReportDir) {
    $ReportDir = Join-Path $RepoRoot 'reports\phase2c_hardware'
}
if (-not (Test-Path $ReportDir)) {
    New-Item -ItemType Directory -Path $ReportDir -Force | Out-Null
}

$ConfigPath = Join-Path $ScriptRoot 'phase2c_hardware_gate_config.json'
if (-not (Test-Path $ConfigPath)) {
    throw "Config file not found at '$ConfigPath'. This script requires tools\phase2c_hardware_gate_config.json to exist alongside it."
}
...
# wall-clock second) never overwrite each other's report - fixes the
# second-granularity collision bug documented in Phase 2A/2B's own hardware
# gate scripts (docs/PHASE2A_HARDWARE_ASSISTED_RESULTS.md,
# docs/PHASE2B_HARDWARE_ASSISTED_RESULTS.md), without modifying either of
# those unrelated, already-accepted scripts.
...
Write-Host ''
Write-Host '=== Phase 2C.4 Hardware-Assisted Validation Gate ===' -ForegroundColor Cyan
...
Write-Host 'app behavior always requires a human running docs/PHASE2C_HARDWARE_SMOKE_TEST_CHECKLIST.md.' -ForegroundColor Yellow
...
$reportObject = [ordered]@{
    ...
    nextActions    = @(
        ...
        'REQUIRES_HUMAN_OBSERVATION entries are not optional to skip - complete docs/PHASE2C_HARDWARE_SMOKE_TEST_CHECKLIST.md on the real device and record results in a copy of docs/PHASE2A_HARDWARE_TEST_RESULTS_TEMPLATE.md.',
        'This report alone never constitutes hardware-tested or release-ready status.'
    )
}

$jsonPath = Join-Path $ReportDir "phase2c_hardware_gate_$RunTimestampForFilename.json"
$reportObject | ConvertTo-Json -Depth 10 | Set-Content -Path $jsonPath -Encoding UTF8

$md = New-Object System.Collections.Generic.List[string]
$md.Add('# Phase 2C.4 Hardware-Assisted Validation Gate Report') | Out-Null
$md.Add('') | Out-Null
...
$mdPath = Join-Path $ReportDir "phase2c_hardware_gate_$RunTimestampForFilename.md"
