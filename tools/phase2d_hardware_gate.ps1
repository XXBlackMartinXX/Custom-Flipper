<#
.SYNOPSIS
    Phase 2D.4 hardware-assisted validation gate for the accepted Phase 2D
    CI baseline (Custom-Flipper, integration/phase2d-first-batch).
...
.DESCRIPTION
    ...
    2D counterpart to tools/phase2a_hardware_gate.ps1,
    tools/phase2b_hardware_gate.ps1, and tools/phase2c_hardware_gate.ps1
    (all three of which remain untouched and still valid for their own
    accepted baselines) - same mechanism, extended to cover all 13 apps now
    in the accepted Phase 2D baseline. `fcc_id_lookup` is not part of this
    baseline (deferred pending a license-evidence resolution) and is not
    referenced anywhere below.

    It validates the Phase 2D baseline recorded in
    tools/phase2d_hardware_gate_config.json (branch, commit, and the real,
    GitHub-Actions-generated artifact hashes from docs/PHASE2D_3_ARTIFACT_HASHES.md)
    ...
    Report-filename collision fix (carried forward from Phase 2C.4): Phase
    2A's and Phase 2B's hardware gate scripts used second-granularity report
    filenames, so two invocations within the same wall-clock second could
    ...
    repeated invocations - the exact same fix Phase 2C.4 introduced,
...
.PARAMETER ArtifactDir
    Path to an already-downloaded-and-extracted Phase 2D CI artifact
...
.PARAMETER ReportDir
    ...
    reports\phase2d_hardware under the repo root. Created if missing.
...
.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\tools\phase2d_hardware_gate.ps1 -Mode Preflight

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\tools\phase2d_hardware_gate.ps1 -Mode HashVerify -ArtifactDir "<artifact-dir>"

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\tools\phase2d_hardware_gate.ps1 -Mode DetectDevice

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\tools\phase2d_hardware_gate.ps1 -Mode HardwareAssisted -ArtifactDir "<artifact-dir>" -AllowFlashPrompt
#>
...
if (-not $ReportDir) {
    $ReportDir = Join-Path $RepoRoot 'reports\phase2d_hardware'
}
if (-not (Test-Path $ReportDir)) {
    New-Item -ItemType Directory -Path $ReportDir -Force | Out-Null
}

$ConfigPath = Join-Path $ScriptRoot 'phase2d_hardware_gate_config.json'
if (-not (Test-Path $ConfigPath)) {
    throw "Config file not found at '$ConfigPath'. This script requires tools\phase2d_hardware_gate_config.json to exist alongside it."
}
...
# Phase 2C.4 introduced (docs/PHASE2C_HARDWARE_ASSISTED_RESULTS.md),
# preserved unchanged here rather than reverting to the Phase 2A/2B
# second-granularity scheme.
...
Write-Host ''
Write-Host '=== Phase 2D.4 Hardware-Assisted Validation Gate ===' -ForegroundColor Cyan
...
Write-Host 'app behavior always requires a human running docs/PHASE2D_HARDWARE_SMOKE_TEST_CHECKLIST.md.' -ForegroundColor Yellow
...
$reportObject = [ordered]@{
    ...
    nextActions    = @(
        ...
        'REQUIRES_HUMAN_OBSERVATION entries are not optional to skip - complete docs/PHASE2D_HARDWARE_SMOKE_TEST_CHECKLIST.md on the real device and record results in a copy of docs/PHASE2A_HARDWARE_TEST_RESULTS_TEMPLATE.md.',
        'This report alone never constitutes hardware-tested or release-ready status.'
    )
}

$jsonPath = Join-Path $ReportDir "phase2d_hardware_gate_$RunTimestampForFilename.json"
$reportObject | ConvertTo-Json -Depth 10 | Set-Content -Path $jsonPath -Encoding UTF8

$md = New-Object System.Collections.Generic.List[string]
$md.Add('# Phase 2D.4 Hardware-Assisted Validation Gate Report') | Out-Null
$md.Add('') | Out-Null
...
$mdPath = Join-Path $ReportDir "phase2d_hardware_gate_$RunTimestampForFilename.md"
