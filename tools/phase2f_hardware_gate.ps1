<#
.SYNOPSIS
    Phase 2F.4 hardware-assisted validation gate for the accepted Phase 2F
    CI baseline (Custom-Flipper, integration/phase2f-first-batch).
...
.DESCRIPTION
    ...
    2F counterpart to tools/phase2a_hardware_gate.ps1,
    tools/phase2b_hardware_gate.ps1, tools/phase2c_hardware_gate.ps1,
    tools/phase2d_hardware_gate.ps1, and tools/phase2e_hardware_gate.ps1
    (all five of which remain untouched and still valid for their own
    accepted baselines) - same mechanism, extended to cover all 19 apps now
    in the accepted Phase 2F baseline. `fcc_id_lookup` is not part of this
    baseline (deferred pending a license-evidence resolution) and is not
    referenced anywhere below.

    It validates the Phase 2F baseline recorded in
    tools/phase2f_hardware_gate_config.json (branch, commit, and the real,
    GitHub-Actions-generated artifact hashes from docs/PHASE2F_3_ARTIFACT_HASHES.md)
    ...
    Report-filename collision fix (carried forward from Phase 2C.4/2D.4/2E.4):
    ...
    repeated invocations - the exact same fix Phase 2C.4/2D.4/2E.4
    introduced, preserved unchanged here. Phase 2A's and Phase 2B's own
    scripts are deliberately left unmodified, per this project's standing
    instruction.
...
.PARAMETER Mode
    ...
    HardwareAssisted    - Combines DetectDevice + HashVerify (if -ArtifactDir
                           ...
                           docs/PHASE2F_HARDWARE_SMOKE_TEST_CHECKLIST.md.
...
.PARAMETER ArtifactDir
    Path to an already-downloaded-and-extracted Phase 2F CI artifact
...
.PARAMETER ReportDir
    ...
    reports\phase2f_hardware under the repo root. Created if missing.
...
.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\tools\phase2f_hardware_gate.ps1 -Mode Preflight

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\tools\phase2f_hardware_gate.ps1 -Mode HashVerify -ArtifactDir "<artifact-dir>"

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\tools\phase2f_hardware_gate.ps1 -Mode DetectDevice

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\tools\phase2f_hardware_gate.ps1 -Mode HardwareAssisted -ArtifactDir "<artifact-dir>" -AllowFlashPrompt
#>
...
if (-not $ReportDir) {
    $ReportDir = Join-Path $RepoRoot 'reports\phase2f_hardware'
}
if (-not (Test-Path $ReportDir)) {
    New-Item -ItemType Directory -Path $ReportDir -Force | Out-Null
}

$ConfigPath = Join-Path $ScriptRoot 'phase2f_hardware_gate_config.json'
if (-not (Test-Path $ConfigPath)) {
    throw "Config file not found at '$ConfigPath'. This script requires tools\phase2f_hardware_gate_config.json to exist alongside it."
}
...
# Phase 2C.4/2D.4/2E.4 introduced, preserved unchanged here rather than
# reverting to the Phase 2A/2B second-granularity scheme.
...
Write-Host ''
Write-Host '=== Phase 2F.4 Hardware-Assisted Validation Gate ===' -ForegroundColor Cyan
...
Write-Host 'app behavior always requires a human running docs/PHASE2F_HARDWARE_SMOKE_TEST_CHECKLIST.md.' -ForegroundColor Yellow
...
$reportObject = [ordered]@{
    ...
    nextActions    = @(
        ...
        'REQUIRES_HUMAN_OBSERVATION entries are not optional to skip - complete docs/PHASE2F_HARDWARE_SMOKE_TEST_CHECKLIST.md on the real device and record results in a copy of docs/PHASE2A_HARDWARE_TEST_RESULTS_TEMPLATE.md.',
        'This report alone never constitutes hardware-tested or release-ready status.'
    )
}

$jsonPath = Join-Path $ReportDir "phase2f_hardware_gate_$RunTimestampForFilename.json"
$reportObject | ConvertTo-Json -Depth 10 | Set-Content -Path $jsonPath -Encoding UTF8

$md = New-Object System.Collections.Generic.List[string]
$md.Add('# Phase 2F.4 Hardware-Assisted Validation Gate Report') | Out-Null
$md.Add('') | Out-Null
...
$mdPath = Join-Path $ReportDir "phase2f_hardware_gate_$RunTimestampForFilename.md"
